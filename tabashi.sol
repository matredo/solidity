...
/* 
*  Website: https://tabashi.space
*  Telegram: https://t.me/tabashiofficial
*
*  Features
*   - 2% Burn on All Transactions
*   - 3% Distrubuted to Dev Wallet
*   - 2% Holder Reward Distribution
...
contract Tabashi is Context, IBEP20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  string private _name = "Tabashi";
  string private _symbol = "TABA";
  ...
  /* 
   * @dev Set DevAddress to a Multi-sig Wallet
  */
  ...
  uint256 public _taxFee = _reflectionFee + _devFee + _burnFee;
  ...
  uint256 private numTokensSellToAddToLiquidity = 100000000000000 * 10**18;
  ...
  constructor(address initialDevAddress) public {
    require(initialDevAddress != address(0), "Address should not be 0x00");
  ...
  */
  ...
  event UpdateDevAddress(address indexed prevDevAddress, address indexed newDevAddress);

  function updateDevAddress(address newAddress) public onlyOwner {
    ...
    emit UpdateDevAddress(prevDevAddress, devAddress);
  }
  ...
  function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    (uint256 devFee, uint256 burnFee, uint256 reflectionFee) = _getTaxBreakdown(tAmount);
  ...
  function setTaxFeePercent(uint256 devFee, uint256 burnFee, uint256 reflectionFee) external onlyOwner() {
    _devFee = devFee;
    _burnFee = burnFee;
    _reflectionFee = reflectionFee;

    _taxFee = devFee + burnFee + reflectionFee;
  }
